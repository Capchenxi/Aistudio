...
from paddle.fluid.layers import softmax, fc

data_path = './data/Dataset'
character_folders = os.listdir(data_path)
# print(character_folders)
if (os.path.exists('./data/train_data.list')):
    os.remove('./data/train_data.list')
if (os.path.exists('./data/test_data.list')):
    os.remove('./data/test_data.list')

for character_folder in character_folders:

    with open('./data/train_data.list', 'a') as f_train:
        with open('./data/test_data.list', 'a') as f_test:
...
# 用于训练的数据提供器
train_reader = paddle.batch(reader=paddle.reader.shuffle(reader=data_reader('./data/train_data.list'), buf_size=256),
                            batch_size=32)
# 用于测试的数据提供器
test_reader = paddle.batch(reader=data_reader('./data/test_data.list'), batch_size=32)
...
class MyDNN(fluid.dygraph.Layer):
    ...
    def forward(self, input):
        ...
        x3 = self.hidden3(x2)
        # print(x3.shape)
        x3 = fluid.layers.reshape(x3, shape=[-1, 3*100*100])
        # print(x3.shape)
        y = self.hidden4(x3)
        # print(y.shape)
        # print("end")
